...
# # Updated code lines to see the training data item
#     # Get a single batch from the training dataset loader
#     train_iter = iter(train_dataset_loader)
#     sample_data = next(train_iter)

#     # Print the sample data structure
#     print("Sample train data format:")
#     if isinstance(sample_data, (tuple, list)):
#         for i, item in enumerate(sample_data):
#             print(f"Item {i}: Type: {type(item)}, Shape: {getattr(item, 'shape', 'N/A')}, Data: {item}")
#     else:
#         print(f"Data Type: {type(sample_data)}, Shape: {getattr(sample_data, 'shape', 'N/A')}, Data: {sample_data}")
    ...
    #     # _z = torch.sign(_z) * (_z.abs() > 0.5).float() # updated code line to apply quantization
    ...
    def train_step(_batch):
        ...
        if torch.isnan(loss).any():
            print("Warning: NaN detected in loss! Skipping this step.")
            return _metrics  # Skip this step if NaN detected

        accelerator.backward(loss.mean())

        # Apply gradient clipping
        torch.nn.utils.clip_grad_norm_(nnet.parameters(), max_norm=1.0) 
