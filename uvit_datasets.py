...
# Start of updated code lines
class FeatureDataset(Dataset):
    def __init__(self, path, num_samples=10_000):  # Updated to 10,000 images
        super().__init__()
        self.path = path
        self.num_samples = num_samples  # Store the number of available samples

    def __len__(self):
        return self.num_samples  # Adjust to the actual number of images
...
# Updated code lines
        for f in sorted(os.listdir(os.path.join(path, 'run_vis')), key=lambda x: int(x.split('.')[0])):
            data = np.load(os.path.join(path, 'run_vis', f), allow_pickle=True).item()
            self.prompts.append(data['prompt'])
            self.contexts.append(data['latent'])
        self.contexts = np.array(self.contexts)
    ...
    def fid_stat(self):
        ...
        # return f'assets/fid_stats/fid_stats_medimgs_train.npz'
        return f'assets/fid_stats/fid_stats_medimgs_val.npz'

### MedImgs data preparation

class MedImgsDatabase(Dataset):
    def __init__(self, root, annFile, size=None):
        from pycocotools.coco import COCO
        self.root = root
        self.height = self.width = size
...
class MedImgsFeatureDataset(Dataset):
    # the image features are got through sample
    def __init__(self, root):
        self.root = root
        self.num_data, self.n_captions = get_feature_dir_info(root)
...
class MedImgs256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder & the contexts calculated by clip
    def __init__(self, path, cfg=False, p_uncond=None):
        super().__init__()
        print('Prepare dataset...')
        self.train = MedImgsFeatureDataset(os.path.join(path, 'train'))
        self.test = MedImgsFeatureDataset(os.path.join(path, 'val'))
        # assert len(self.train) == 82783
        # assert len(self.test) == 40504
        print('Prepare dataset ok')
...
def get_dataset(name, **kwargs):
    ...
    elif name == 'medimgs256_features':
        return MedImgs256Features(**kwargs)        
    else:
